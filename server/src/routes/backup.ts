...
export function backupRoutes(opts: {
  ...
  router.post("/trigger", async (_req, res, next: NextFunction) => {
    try {
      const scheduler = opts.getScheduler();
      if (!scheduler) {
  ...
  router.post("/:name/restore", async (req, res, next: NextFunction) => {
    try {
      const { name } = req.params;
  ...
  router.delete("/:name", async (req, res, next: NextFunction) => {
    try {
      const { name } = req.params;
