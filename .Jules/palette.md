...
**Learning:** Found multiple instances where `size="icon"` or similar buttons in the UI lacked `aria-label` attributes.
**Action:** Always ensure that buttons conveying information solely through visual icons include an `aria-label` for screen reader accessibility.
