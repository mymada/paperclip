...
import { listFullBackups, runDatabaseBackup, runDatabaseRestore, runFullBackup } from "./backup-lib.js";
...
describe("backup-lib", () => {
    ...
    describe("runFullBackup", () => {
        it("creates a full backup directory with manifest, db file, and copied instance files", async () => {
        ...
        it("skips included dirs that do not exist on disk without failing", async () => {
            const support = await getEmbeddedPostgresTestSupport();
            if (!support.supported) {
                console.warn("Embedded Postgres not supported, skipping test: " + support.reason);
                return;
    ...
    describe("database backup/restore", () => {
        it("can backup and restore a database with data, enums, and sequences", async () => {
            const support = await getEmbeddedPostgresTestSupport();
            if (!support.supported) {
                console.warn("Embedded Postgres not supported, skipping test: " + support.reason);
                return;
