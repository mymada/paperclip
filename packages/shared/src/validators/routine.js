...
const createRoutineTriggerSchema = z.discriminatedUnion("kind", [
  ...
  baseTriggerSchema.extend({
    ...
    replayWindowSec: z.number().int().min(30).max(86400).optional().default(300)
  }),
  baseTriggerSchema.extend({
    kind: z.literal("api")
  })
]);
const updateRoutineTriggerSchema = z.object({
  label: z.string().trim().max(120).optional().nullable(),
  ...
  replayWindowSec: z.number().int().min(30).max(86400).optional().nullable()
});
...
const rotateRoutineTriggerSecretSchema = z.object({});
export {
  createRoutineSchema,
